...
export default function ClientLayout({ children }) {
  ...
  const navigation = [
    ...
    { name: 'Bandeja de Entrada', href: '/inbox', icon: InboxIcon, badge: unreadCount },
      ...
      <header className="fixed top-0 left-0 right-0 h-16 bg-white border-b border-gray-200 z-50">
        <div className="h-full px-4 lg:px-8 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {/* Notification Bell - Mejorado para mayor visibilidad */}
            <Link 
              to="/inbox" 
              className={`relative p-2 rounded-lg transition-all ${
                unreadCount > 0 
                  ? 'text-primary-600 bg-primary-50 hover:bg-primary-100 ring-2 ring-primary-200' 
                  : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'
              } ${bellAnimating ? 'animate-shake-bell' : ''}`}
              title={unreadCount > 0 ? `${unreadCount} mensajes sin leer` : 'Bandeja de entrada'}
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
              </svg>
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 px-2 py-0.5 text-xs font-bold bg-red-500 text-white rounded-full min-w-[22px] text-center animate-pulse shadow-lg ring-2 ring-white">
                  {unreadCount > 99 ? '99+' : unreadCount}
                </span>
              )}
            </Link>
            
            {/* Indicador de mensajes no leídos - Solo visible con mensajes pendientes */}
            {unreadCount > 0 && (
              <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-red-50 border border-red-200 rounded-full">
                ...
                <span className="text-xs font-semibold text-red-700">
                  {unreadCount} {unreadCount === 1 ? 'mensaje' : 'mensajes'}
                </span>
              </div>
            )}
